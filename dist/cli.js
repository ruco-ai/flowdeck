...
// -- worktree lifecycle -------------------------------------------------------
const activeWorktreeSlugs = new Set();
function worktreePath(root, slug) {
    return join(root, '.git', 'flowdeck-tmp', ...slug.split('/'));
}
async function createWorktree(slug, root) {
...
async function removeWorktree(slug, root) {
    try {
        const wtPath = worktreePath(root, slug);
...
function cleanupAllWorktrees(root) {
...
class ParallelDisplay {
    ...
    render() {
        if (this.rendered)
            process.stdout.write(`\x1b[${this.slugs.length}A`);
...
function spawnClaudeExecutor(slug, args, cwd, display, logPath) {
...
// -- prompts ------------------------------------------------------------------
function buildPlayPrompt(agentMd, cardPath, cardContent, slug) {
...
function buildTurnPrompt(agentMd, cardCount, hand) {
...
async function runOrchestrator(root, cards, deps) {
...
async function executeParallelGroup(root, slugs, cards, agentMd) {
...
function mergeBranch(slug, root) {
    ...
    try {
        ...
        let conflicting = '';
        try {
            conflicting = execSync('git diff --name-only --diff-filter=U', { cwd: root }).toString().trim();
        }
        catch { }
...
async function turnCommand(args) {
    ...
    for (const card of cards) {
        ...
        if (deps.length > 0)
            process.stderr.write(`deps: ${card.slug} → ${deps.join(', ')}\n`);
    }
