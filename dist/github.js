...
export function parseFrontmatter(content) {
    ...
    for (const line of m[1].split('\n')) {
        ...
        const [, k, v] = kv;
        if (k === 'github_issue')
            fm.github_issue = v.trim();
        if (k === 'github_synced_at')
            fm.github_synced_at = v.trim();
        if (k === 'github_labels') {
...
export function writeFrontmatter(cardPath, key, value) {
...
export function parseCard(cardPath, content) {
...
async function ghFetch(token, method, path, body, verbose) {
    const url = `https://api.github.com${path}`;
    if (verbose)
        process.stderr.write(`[gh] ${method} ${url}\n`);
    const opts = {
        method,
        headers: {
...
function getToken(opts) {
    ...
    if (!token) {
        process.stderr.write('GITHUB_TOKEN not found. Set env var or use: op read "op://Personal/GitHub/flowdeck-token"\n');
        process.exit(1);
    }
    return token;
}
function parseIssueRef(ref) {
    ...
    if (withNum)
        return { owner: withNum[1], repo: withNum[2], number: parseInt(withNum[3], 10) };
    const noNum = ref.match(/^([^/]+)\/([^#\s]+)$/);
    if (noNum)
        return { owner: noNum[1], repo: noNum[2] };
    throw new Error(`Invalid github_issue format: "${ref}". Expected "owner/repo#123" or "owner/repo"`);
}
// -- Public actions -----------------------------------------------------------
export async function createIssue(owner, repo, card, token, dryRun, verbose) {
...
export async function postBotComment(owner, repo, issueNumber, card, cardPath, token, dryRun, verbose) {
...
async function ensureLabel(owner, repo, label, token, verbose) {
    const colors = {
    ...
    const res = await ghFetch(token, 'POST', `/repos/${owner}/${repo}/labels`, { name: label, color: colors[label] ?? 'EDEDED' }, verbose);
    if (!res.ok && res.status !== 422 && verbose) {
        process.stderr.write(`warn: could not create label "${label}": ${res.status}\n`);
    }
}
export async function transitionIssue(owner, repo, issueNumber, phase, token, dryRun, verbose) {
    const addLabel = {
    ...
    for (const label of toRemove) {
        const res = await ghFetch(token, 'DELETE', `/repos/${owner}/${repo}/issues/${issueNumber}/labels/${encodeURIComponent(label)}`, undefined, verbose);
        if (!res.ok && res.status !== 404 && verbose) {
            process.stderr.write(`warn: could not remove label "${label}": ${res.status}\n`);
        }
    }
}
export async function closeIssue(owner, repo, issueNumber, token, dryRun, verbose) {
    if (dryRun) {
    ...
    if (!ref.number) {
        ...
        if (!opts.dryRun) {
            try {
                ...
                process.stderr.write(`Could not write issue number back to card. Add manually: github_issue: ${ref.owner}/${ref.repo}#${issueNumber}\n`);
                process.exit(1);
            }
        }
