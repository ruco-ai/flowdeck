...
function uniqueSlug(base: string, dir: string): string {
  ...
  let i = 2
  while (existsSync(join(dir, `${base}-${i}`))) i++
  return `${base}-${i}`
}

function tree(dir: string, indent = ''): string {
...
function getFileDiff(dir: string): string {
...
server.defineTool({
  name: 'open',
  description:
    'Open a new issue — creates open/<slug>/README.md with the given title and body. ' +
  ...
  handler: async ({ title, body, parent }) => {
    ...
    if (parent && !existsSync(parentDir)) throw new Error(`Parent "${parent}" not found in open/`)
...
server.defineTool({
  name: 'close',
  description:
    'Close an issue by moving it from open/ to done/. ' +
  ...
  handler: async ({ path }) => {
    ...
    if (!existsSync(src)) throw new Error(`Issue "${path}" not found in open/`)
...
server.defineTool({
  ...
  handler: async ({ path, content }) => {
    ...
    if (!existsSync(readme)) throw new Error(`Issue "${path}" not found in open/`)
...
server.defineTool({
  name: 'flowdeck-do',
  description:
    'Get context about what the human asked (reads last commit), or finalize work by committing. ' +
  ...
  handler: async ({ path, action, message }) => {
    ...
    if (!existsSync(issueDir)) throw new Error(`Issue "${path}" not found in open/`)
    ...
    if (action === 'commit') {
      if (!message) throw new Error('message is required for action="commit"')
